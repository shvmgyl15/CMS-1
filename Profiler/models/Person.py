...
class MobileManager(models.Manager):
    ...
    def editContacts(self,request):
        '''This function is to edit the contacts in the data base
        The id used for personalMobile And alternativeMobile is perMobId and altMobId respectively'''
        M = Mobile.objects.get(id = request['perMobId'])
        M.countryCode = request['personalMobile']['countryCode']
        M.mobileNum = request['personalMobile']['mobileNum']
        M.save()
        #this is the list of the objects of the personal and alternative mobiles so that
        #we can return the changed objects at the end of the function
        contactObjs = []
        contactObjs.append(M)
        M = Mobile.objects.get(id = request['altMobId'])
        M.countryCode = request['alternativeMobile']['countryCode']
        M.mobileNum = request['alternativeMobile']['mobileNum']
        M.save()
        contactObjs.append(M)
